...
# Pause overlay button zones in 320x240 space
...
class ScreenManager(object):
    ...
    def __init__(self):
        ...
        self.game         = None
        self._activeLevel = None   # "level1" / "level2" / "level3"
        self._activeMap   = None   # mapFile string for restart

        # Placeholder text for level 2
    ...
    def draw(self, drawSurf):
        if self.state == "mainMenu":
            ...
            # Draw the frozen game underneath, then overlay
            self.game.draw(drawSurf)
    ...
    def handleEvent(self, event):
        ...
        # ── Paused ─────────────────────────────────────────────────────
            ...
            if mouseHit(event, RECT_RESUME):
                ...
                return   # prevent event from reaching mainMenu handler
    ...
    def update(self, seconds):
        # Game only updates when NOT paused
        if self.state in ["level1", "level3"] and self.game:
    ...
    def _restartLevel(self):
        """Destroy current game, create fresh one, resume same level."""
